...
class TfmdDL(DataLoader):
    ...
    def __init__(self,
        ...
        num_workers:int|None=None, # Number of CPU cores to use in parallel (default: All available up to 16)
        verbose:bool=False, # Print verbose logs
    ...
    def show_batch(self,
        b=None, # Batch to show, defaults to `one_batch`
        ...
        show:bool=True, # Show or return decoded items
        unique:bool=False, # Show only one item
        **kwargs
    ):
        "Show `max_n` input(s) and target(s) from the batch."
        if unique:
    ...
    def show_results(self,
        b, # Input batch
        out, # Predicted output from model
        max_n:int=9, # Maximum number of results to show
        ctxs=None, # List of `ctx` objects to show data. Could be matplotlib axis, DataFrame etc
        show:bool=True, # Show or return decoded results
        **kwargs
    ):
        "Show `max_n` results with input(s), target(s) and prediction(s)."
    ...
    def to(self, device:torch.device|str|None):
        "Move `DataLoader` and `after_batch` transforms to `device`"
        self.device = device
...
class DataLoaders(GetAttr):
    ...
    def __init__(self,
        *loaders, # `DataLoader` objects to wrap
        ...
        device:torch.device|str|None=None # Device to put `DataLoaders`
    ):
        self.loaders,self.path = list(loaders),Path(path)
        if device is not None or hasattr(loaders[0],'to'): self.device = device

    def __getitem__(self, i:int): return self.loaders[i]
    ...
    @device.setter
    def device(self, d:torch.device|str|None):
        "Move `DataLoader`s in `DataLoaders` to `device`"
    ...
    def add_tfms(self,
        ...
        loaders:listified[int|str]|None=None # Index or name of `DataLoader` object(s) to add `tfms`
    ):
    ...
    def from_dsets(cls,
        *ds, # `Datasets` object(s)
        ...
        bs:int=64, # Batch size
        device:torch.device|str|None=None, # Device to put `DataLoaders`
        dl_type:Callable[...,TfmdDL]=TfmdDL, # Type of `DataLoader`
        **kwargs
    ):
    ...
    def from_dblock(cls,
        ...
        path:str|Path='.', # Path to put in `DataLoaders`
        bs:int=64, # Batch size
        val_bs:int|None=None, # Batch size for validation `DataLoader`
...
class FilteredBase:
    ...
    def dataloaders(self,
        bs:int=64, # Batch size
        shuffle_train:bool|None=None, # (Deprecated, use `shuffle`) Shuffle training `DataLoader`
        shuffle:bool=True, # Shuffle training `DataLoader`
        val_shuffle:bool=False, # Shuffle validation `DataLoader`
        n:int|None=None, # Size of `Datasets` used to create `DataLoader`
        path:str|Path='.', # Path to put in `DataLoaders`
        dl_type:TfmdDL|None=None, # Type of `DataLoader`
        dl_kwargs:listy[dict]|None=None, # kwargs to pass to individual `DataLoader`s
        device:str|torch.device|None=None, # Device to put `DataLoaders`
        drop_last:bool|None=None, # Drop last incomplete batch, defaults to `shuffle`
        val_bs:int|None=None, # Validation batch size, defaults to `bs`
        **kwargs
    ) -> DataLoaders:
...
class TfmdLists(FilteredBase, L, GetAttr):
    ...
    _default='tfms'
    def __init__(self,
        items, # Items to apply `tfms` to
        tfms:listified[Any], # Transform(s) for `items`. Must be compatible with `Transform`, `Pipeline`, and/or `TfmdLists`
        use_list:bool|None=None, # Use `list` in `L`
        do_setup:bool|None=True, # Call `setup` for `tfms`
        split_idx:int|None=None, # Apply `tfms` to training set `0`, validation set `1`, or both `None`
        train_setup:bool=True, # Apply `tfms` only on training `DataLoader`
        splits:listy[int|bool]|listy[listy[int|bool]]|None=None, # Indices for training and validation sets
        types:Any|None=None, # Types of data in `items`
        verbose:bool=False, # Print verbose output
        dl_type:Callable[...,TfmdDL]|None=None # Type of `DataLoader`
    ):
...
class Datasets(FilteredBase):
    ...
    def __init__(self,
        items:listy[Any]|None=None, # List of items to create `Datasets`
        tfms:listified[listified[Any]]|None=None, # Transform(s) for `items` if `tls` is None. Must be compatible with `Transform`, `Pipeline`, and/or `TfmdLists`
        tls:listified[TfmdLists]|None=None, # If None, `self.tls` is generated from `items` and `tfms`
        n_inp:int|None=None, # Number of elements in `Datasets` tuple that should be considered part of input
...
def test_set(
    ...
    test_items:Any, # Items in test dataset
    rm_tfms:listified[int]|None=None, # Start index of `Transform`(s) from validation set in `dsets` to apply
...
def test_dl(self:DataLoaders,
    test_items:Any, # Items in test dataset
    rm_type_tfms:listified[int]|None=None, # Start index of `Transform`(s) from validation set in `dsets` to apply
    with_labels:bool=False, # Whether the test items contain labels
    **kwargs
):
