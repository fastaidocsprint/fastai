...
class RandTransform(DisplayedTransform):
    ...
    def __init__(self,
        ...
        nm:str|None=None,
        before_call:Callable|None=None, # Optional batchwise preprocessing function
        **kwargs
    ):
        store_attr('p')
...
def crop_pad(x:(TensorBBox,TensorPoint,Image.Image),
    ...
    tl:tuple[int,int]|None=None, # Optional top-left coordinate of the crop/pad, if `None` center crop
    orig_sz:tuple[int,int]=None, # Original size of input
    pad_mode:PadMode=PadMode.Zeros, # Fastai padding mode
    resize_mode=Image.BILINEAR, # Pillow `Image` resize mode
    resize_to:tuple[int,int]|None=None # Optional post crop/pad resize of input
):
...
class CropPad(DisplayedTransform):
    ...
    order = 0
    def __init__(self,
        size:int|tuple[int,int], # Size to crop or pad to, duplicated if one value is specified
...
class RandomCrop(RandTransform):
    ...
    def __init__(self,
        size:int|tuple[int,int], # Size to crop to, duplicated if one value is specified
        **kwargs
    ):
        size = _process_sz(size)
        store_attr()
...
class Resize(RandTransform):
    ...
    def __init__(self,
        size:int|tuple[int,int], # Size to resize to, duplicated if one value is specified
...
class RandomResizedCrop(RandTransform):
    ...
    split_idx,order = None,1
    def __init__(self,
         size:int|tuple[int,int], # Final size, duplicated if one value is specified,,
         min_scale:float=0.08, # Minimum scale of the crop, in relation to image area
         ratio:tuple[float,float]=(3/4, 4/3), # Range of width over height of the output
...
def affine_grid(
    ...
    align_corners:bool|None=None # PyTorch `F.grid_sample` align_corners
):
...
def affine_coord(x: TensorImage,
     mat:Tensor|None=None, # Batch of affine transformation matrices
     coord_tfm:Callable[[Tensor], Tensor]|None=None, # Partial function of composable coordinate transforms
     sz:int|tuple[int,int]|None=None, # Output size, duplicated if one value is specified
...
def affine_coord(x: TensorMask,
    mat:Tensor|None=None, # Batch of affine transformation matrices
    coord_tfm:Callable[[Tensor], Tensor]|None=None, # Partial function of composable coordinate transforms
    sz:int|tuple[int,int]=None, # Output size, duplicated if one value is specified
...
class AffineCoordTfm(RandTransform):
    ...
    def __init__(self,
        aff_fs:listified[Callable[[Tensor], Tensor]]|None=None, # Affine transformations function for a batch
        coord_fs:listified[Callable[[Tensor], Tensor]]|None=None, # Coordinate transformations function for a batch
        size:int|tuple[int,int]|None=None, # Output size, duplicated if one value is specified
...
class RandomResizedCropGPU(RandTransform):
    ...
    def __init__(self,
        ...
        ratio:tuple[float,float]=(3/4, 4/3), # Range of width over height of the output
        mode='bilinear', # PyTorch `F.grid_sample` interpolation
        valid_scale:float=1., # Scale of the crop for the validation set, in relation to image area
        max_scale:float=1., # Maximum scale of the crop, in relation to image area
        **kwargs
    ):
        if isinstance(size, int): size = (size,size)
...
def flip_mat(
    ...
    draw:listified[int]|Callable[[], listy[int]]|None=None, # Custom flips instead of random
...
def flip_batch(x: (TensorImage,TensorMask,TensorPoint,TensorBBox),
    ...
    draw:listified[int]|Callable[[], listy[int]]|None=None, # Custom flips instead of random
    size:int|tuple[int,int]|None=None, # Output size, duplicated if one value is specified
    mode:str|None=None, # PyTorch `F.grid_sample` interpolation applied to `x`
...
def dihedral_mat(
    ...
    draw:listified[int]|Callable[[], listy[int]]|None=None, # Custom dihedrals instead of random
...
def rotate_mat(
    ...
    draw:listified[int]|Callable[[], listy[int]]|None=None, # Custom rotates instead of random
...
def zoom_mat(
    ...
    draw:listified[int]|Callable[[], listy[int]]|None=None, # User defined scale of the zoom
    draw_x:listified[int]|Callable[[], listy[int]]|None=None, # User defined center of the zoom in x
    draw_y:listified[int]|Callable[[], listy[int]]|None=None, # User defined center of the zoom in y
...
def warp(x:(TensorImage,TensorMask,TensorPoint,TensorBBox),
    size:int|tuple[int,int]=None, # Output size, duplicated if one value is specified
    mode:str='bilinear', # PyTorch `F.grid_sample` interpolation applied to `x`
    pad_mode=PadMode.Reflection, # Padding applied to `x`
    align_corners:bool=True, # PyTorch `F.grid_sample` align_corners
    **kwargs
...
class Warp(AffineCoordTfm):
    ...
    def __init__(self,
        ...
        draw_x:listified[float]|Callable[[], listy[float]]|None=None, # User defined warping magnitude in x
        draw_y:listified[float]|Callable[[], listy[float]]|None=None, # User defined warping magnitude in y
        size:int|tuple[int,int]=None, # Output size, duplicated if one value is specified
        mode:str='bilinear', # PyTorch `F.grid_sample` interpolation
        pad_mode=PadMode.Reflection, # A `PadMode`
...
class SpaceTfm(RandTransform):
    ...
    order = 40
    def __init__(self,
        fs:listified[Callable[[Tensor],Tensor]], # Transformation functions applying in a space
        space_fn:Callable[[Tensor],Tensor], # Function converting rgb to a space and back to rgb after appying `fs`
        **kwargs
    ):
        super().__init__(**kwargs)
    ...
    def compose(self,
        tfm:Callable[[Tensor],Tensor] # Transformation function to compose
    ):
...
class LightingTfm(SpaceTfm):
    ...
    order = 40
    def __init__(self,
        fs:listified[Callable[[Tensor],Tensor]], # Transformation functions applying in logit space,
        **kwargs
    ):
...
class Brightness(LightingTfm):
    def __init__(self,
        ...
        draw:listified[float]|Callable[[], listy[float]]|None=None, # User defined behavior of batch transformation
...
def cutout_gaussian(
    ...
    areas:list[int,int,int,int] # List of areas to cutout. Order rl,rh,cl,ch
):
...
def norm_apply_denorm(
    x:Tensor, # Input Image
    f:Callable[[Tensor],Tensor], # Function to apply
    nrm:Callable[[Tensor],Tensor] # Normalization transformation
):
...
def aug_transforms(
    ...
    xtra_tfms:list|None=None, # Custom Transformations
    size:int|tuple[int,int]|None=None, # Output size, duplicated if one value is specified
    mode:str='bilinear', # PyTorch `F.grid_sample` interpolation
    pad_mode=PadMode.Reflection, # A `PadMode`
    align_corners:bool=True, # PyTorch `F.grid_sample` align_corners
    batch:bool=False, # Apply identical transformation to entire batch
    min_scale:float=1. # Minimum scale of the crop, in relation to image area
):
