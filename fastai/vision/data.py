...
def get_grid(
    ...
    nrows:int|None=None, # Number of rows in the returned grid, defaulting to `int(math.sqrt(n))`
    ncols:int|None=None, # Number of columns in the returned grid, defaulting to `ceil(n/rows)`
    figsize:tuple|None=None, # Width, height in inches of the returned figure
    double:bool=False, # Double the number of columns and `n`
    title:str=None, # Figure title
    return_fig:bool=False, # Return the figure created by `subplots`
    flatten:bool=True, # Flatten the matplot axes such that they can be iterated over with a single loop
    **kwargs,
...
# Cell
def MaskBlock(
    codes:list|None=None # Vocab labels for segmentation masks
):
...
# Cell
def BBoxLblBlock(
    vocab:list|None=None, # Vocab labels for bounding boxes
